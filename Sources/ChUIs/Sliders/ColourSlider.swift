...
open class BiDirectionalColourSlider: UIControl {
    ...
    @IBInspectable var minimumValue: CGFloat = -1 { didSet { updateLayerFrames() } }
    @IBInspectable var maximumValue: CGFloat = 1 { didSet { updateLayerFrames() } }
    @IBInspectable var value: CGFloat = 0 { didSet { updateLayerFrames() } }
    @IBInspectable var sliderBackgroundColour: UIColor = .systemBackground { didSet { updateLayerFrames() } }
...
open class BiDirectionalColourSliderTrackLayer: CALayer {
  weak var slider: BiDirectionalColourSlider?
    ...
    open override func draw(in ctx: CGContext) {
        ...
        let position = slider.fullRangePositionForValue(slider.value)
        if slider.value > 0 {
            let upperHeight = (bounds.height / 2) * (position / bounds.height)
            print("upperHeight \(upperHeight)")
        } else if slider.value < 0 {
            let lowerHeight = (bounds.height / 2) * ((position * -1) / bounds.height)
            print("lowerHeight \(lowerHeight)")
        } else {
            print("0 time")
        }
        let minimumHeight = 3.0
        let height = max((bounds.height * max(position, 1.0)), minimumHeight)
